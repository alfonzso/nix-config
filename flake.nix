    ...
    let
      ...
      mkHost = flakeConfigName: {
        ${flakeConfigName} = let
          ...
          hostNixpkgs =
            if useNextRelease then inputs.nixpkgs_26_05 else nixpkgs;
          hostHomeManager =
            if useNextRelease then inputs.home-manager_26_05 else home-manager;
          hostSopsNix =
            if useNextRelease then inputs.sops_nix_26_05 else sops-nix;
